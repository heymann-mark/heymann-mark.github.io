...
async function astarAlgorithm(src, dest) {
  ...
  while (!pq.isEmpty()) {
    ...
    if (resetOn === 0) {
      ...
      v = u - 1; //left
      if (v > columns * Math.floor(u / columns) - 1 && visited.get(v) === 0) {
        //left
      ...
      if (v > 0) {
        ...
        if (
          dist.get(u) + weight + h(v, dest) < dist.get(v) &&
          visited.get(v) === 0
        ) {
          dist.set(v, dist.get(u) + weight);
          previousVertex.set(v, u);
      ...
      v = u - columns - 1;
      if (v > columns * Math.floor(u / columns - 1) - 1 && v > 0) {
        //upleft
      ...
      v = u - columns + 1;
      if (
        v < columns * Math.floor(u / columns) &&
        v > 0 &&
        visited.get(v) === 0
      ) {
        //upright
      ...
      v = u + columns + 1;
      if (v < columns * Math.floor(u / columns + 2)) {
        //rightdown
      ...
      if (u !== src && u !== dest) boxes[u].style.backgroundColor = "blue";
    }
    if (u === dest) {
      while (!pq.isEmpty()) {
        pq.dequeue();
...
function h(n, dest) {
  //manhattan
  return (
    Math.abs((n % columns) - (dest % columns)) +
    Math.abs(Math.floor(n / columns) - Math.floor(dest / columns))
  );
}
...
function euclidean(src, dest) {
  return Math.sqrt(
    Math.pow(xstart - xdestination, 2) + Math.pow(ystart - ydestination, 2)
  );
}
