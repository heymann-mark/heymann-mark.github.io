...
async function dijkstra(src, dest) {
  ...
  dijkstrasAlgorithm(src, dest).then();
}

async function dijkstrasAlgorithm(src, dest) {
  dist.set(src, 0);
  pq.enqueue(src, 1);
  while (!pq.isEmpty()) {
    ...
    if (v < columns * rows) {
      ...
      if (
        parseFloat(dist.get(u)) + parseFloat(weight) <
        parseFloat(dist.get(v))
      ) {
    ...
    v = u - columns - 1;
    if (v > columns * Math.floor(u / columns - 1) && v > 0) {
      //upleft
    ...
    v = u - columns + 1;
    if (v < columns * Math.floor(u / columns) && v > 0) {
      //upright
      weight = weightmaps.get(v).get(v + columns - 1);
      if (dist.get(u) + weight < dist.get(v)) {
        dist.set(v, dist.get(u) + 1);
